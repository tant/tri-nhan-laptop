...
import { vietnameseTestData, databaseTestUtils } from '../../utils/vietnamese-test-helpers'
    ...
    it('should handle database connection failures gracefully', async () => {
      ...
      mockSupabaseClient.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          single: vi.fn().mockRejectedValue(new Error('Kết nối cơ sở dữ liệu thất bại'))
        })
      })
  ...
  describe('1.1.2-INT-001: Ticket code uniqueness across years', () => {
    it('should ensure ticket codes remain unique across different years', async () => {
      ...
      const generateAndStoreTicketCode = async (year: number) => {
        ...
        const sequence = (timestamp + uniqueId).toString().slice(-6).padStart(6, '0')
      ...
      for (const user of vietnameseUsers) {
        const createProfile = async (authUser: any) => {
          return {
            ...
            name_has_vietnamese: /[àáạảãăằắặẳẵâầấậẩẫèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđĐ]/.test(authUser.user_metadata.full_name),
            encoded_correctly: encodeURIComponent(authUser.user_metadata.full_name) !== authUser.user_metadata.full_name
          }
        }
    ...
    it('should correctly store and retrieve Vietnamese text data', async () => {
      ...
      for (const testCase of vietnameseTestCases) {
        ...
        const storeVietnameseData = async (data: any) => {
          ...
          return {
            stored: decoded,
            encoding_preserved: decoded.name === data.name &&
    ...
    it('should handle Vietnamese text sorting correctly', async () => {
      ...
      const sortVietnamese = (names: string[]) => {
        ...
        return [...names].sort((a, b) => {
          ...
          return removeVietnameseTones(a).localeCompare(removeVietnameseTones(b))
        })
      }
