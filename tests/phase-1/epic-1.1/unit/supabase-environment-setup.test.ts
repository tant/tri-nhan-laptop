...
import { environmentValidation, vietnameseTestData } from '../../utils/vietnamese-test-helpers'
...
describe('Epic 1.1.1: Supabase Environment Setup - Unit Tests', () => {
  ...
  describe('1.1.1-UNIT-001: Database schema validation', () => {
    ...
    it('should validate Vietnamese character encoding support', () => {
      ...
      testNames.forEach(name => {
        ...
        expect(name).toMatch(/[àáạảãăằắặẳẵâầấậẩẫèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđĐ]/)
    ...
    it('should validate required database tables structure', () => {
      ...
      const validateTableStructure = (tableName: string) => {
        const schemas = {
          customers: ['id', 'phone', 'name', 'address', 'created_at', 'updated_at'],
          repair_tickets: ['id', 'ticket_code', 'customer_id', 'device_info', 'issue_description', 'status'],
  ...
  describe('1.1.1-UNIT-002: Environment variables loading', () => {
    ...
    it('should validate environment variable format', () => {
      ...
      expect(testKey).toMatch(/^[A-Za-z0-9\-_]+\.?[A-Za-z0-9\-_]*\.?[A-Za-z0-9\-_]*$/)
    })
