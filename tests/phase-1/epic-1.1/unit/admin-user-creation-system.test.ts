...
describe('Epic 1.1.3: Admin User Creation & Development Utilities - Unit Tests', () => {
  ...
  describe('1.1.3-UNIT-001: Environment variable parsing', () => {
    it('should parse admin user configuration from environment variables', () => {
      ...
      const requiredFields = ['ADMIN_EMAIL', 'ADMIN_PASSWORD', 'ADMIN_NAME', 'ADMIN_PHONE']

      requiredFields.forEach(field => {
        expect(mockEnvConfig[field as keyof typeof mockEnvConfig]).toBeDefined()
      ...
      expect(vietnameseTextValidation.isValidVietnameseName(mockEnvConfig.ADMIN_NAME)).toBe(true)
    ...
    it('should provide default values for missing optional fields', () => {
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(config.name)).toBe(true)
    })
  ...
  describe('1.1.3-UNIT-002: Sample data structure validation', () => {
    it('should validate Vietnamese sample customer data structure', () => {
      ...
      sampleCustomers.forEach(customer => {
        ...
        expect(vietnameseTextValidation.isValidVietnameseName(customer.name)).toBe(true)
        expect(phoneValidation.validate(customer.phone)).toBe(true)
        expect(vietnameseTextValidation.hasVietnameseCharacters(customer.address)).toBe(true)
    ...
    it('should validate sample repair ticket data with Vietnamese descriptions', () => {
      const sampleRepairTickets = [
        {
          ...
          issue_description: 'Máy tính bị chậm, cần nâng cấp RAM và làm sạch quạt tản nhiệt',
          status: 'RECEIVED',
      ...
      sampleRepairTickets.forEach(ticket => {
        ...
        // Validate status
        const validStatuses = ['RECEIVED', 'IN_PROGRESS', 'WAITING_PARTS', 'COMPLETED', 'CANCELLED']
  ...
  describe('1.1.3-UNIT-003: Health check logic validation', () => {
    it('should validate system component health check logic', () => {
      ...
      return performHealthCheck().then(results => {
        ...
        const services = ['database', 'auth_service', 'storage_service', 'api_gateway']
        services.forEach(service => {
          expect(results.checks).toHaveProperty(service)
          expect(results.checks[service as keyof typeof results.checks]).toHaveProperty('status')
          expect(results.checks[service as keyof typeof results.checks]).toHaveProperty('response_time')
        })
      })
    })

    it('should detect and report unhealthy services', () => {
      const assessHealthStatus = (checks: Record<string, { status: string; response_time: number }>) => {
        const healthyServices = Object.entries(checks).filter(([_, check]) => check.status === 'healthy')
        const unhealthyServices = Object.entries(checks).filter(([_, check]) => check.status !== 'healthy')

        return {
          overall_status: unhealthyServices.length === 0 ? 'healthy' : 'degraded',
  ...
  describe('1.1.3-UNIT-004: Role assignment logic validation', () => {
    it('should assign correct default roles for new users', () => {
      const assignUserRole = (userMetadata: any, isFirstUser: boolean = false) => {
  ...
  describe('Edge cases and error handling', () => {
    it('should handle invalid Vietnamese names gracefully', () => {
      ...
      invalidNames.forEach(name => {
        if (name.length === 0) {
          expect(vietnameseTextValidation.isValidVietnameseName(name)).toBe(false)
    ...
    it('should handle missing environment variables with appropriate defaults', () => {
      ...
      expect(() => createAdminFromEnv({})).toThrow('ADMIN_EMAIL và ADMIN_PASSWORD là bắt buộc')
