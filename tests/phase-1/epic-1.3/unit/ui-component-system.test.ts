...
import { vietnameseTestData, vietnameseTextValidation } from '../../utils/vietnamese-test-helpers'
...
describe('Epic 1.3.3: UI Component System - Unit Tests', () => {
  ...
  describe('1.3.3-UNIT-001: Radix UI component configuration', () => {
    it('should validate Radix UI component props with Vietnamese content', () => {
          ...
          return {
            ...
            children: [
              ...
              { type: 'footer', children: [
                { type: 'button', props: { variant: 'outline' }, children: this.content.cancelText },
                { type: 'button', props: { variant: 'destructive' }, children: this.content.confirmText }
              ]}
            ]
          }
        }
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(mockDialog.content.title)).toBe(true)
      expect(vietnameseTextValidation.hasVietnameseCharacters(mockDialog.content.description)).toBe(true)
      expect(vietnameseTextValidation.hasVietnameseCharacters(mockDialog.content.cancelText)).toBe(true)
    ...
    it('should handle Vietnamese dropdown menu options', () => {
      ...
      mockDropdownMenu.options.forEach(option => {
        expect(vietnameseTextValidation.hasVietnameseCharacters(option.label)).toBe(true)
      })
    ...
    it('should validate Vietnamese form components', () => {
      ...
      vietnameseFormFields.forEach(field => {
        expect(vietnameseTextValidation.hasVietnameseCharacters(field.label)).toBe(true)
        if (field.placeholder) {
          expect(vietnameseTextValidation.hasVietnameseCharacters(field.placeholder)).toBe(true)
        }
      })
      ...
      const validFormData = {
        ...
        issueDescription: 'Laptop bị chậm, quạt kêu to, cần làm sạch và bảo trì'
      }
      ...
      Object.values(invalidResult.errors).forEach(errorMessage => {
        expect(vietnameseTextValidation.hasVietnameseCharacters(errorMessage)).toBe(true)
      })
    })
  })
  ...
  describe('1.3.3-UNIT-002: Tailwind CSS Vietnamese text rendering', () => {
    it('should render Vietnamese text with proper typography classes', () => {
      ...
      const textRenderer = {
        ...
        checkTextOverflow: (text: string, maxWidth: number) => {
          ...
          return {
            ...
            truncatedText: approximateWidth > maxWidth
      ...
      Object.entries(vietnameseTextSamples).forEach(([variant, text]) => {
        const rendered = textRenderer.renderText(text, variant as keyof typeof typographyClasses)

        expect(rendered.children).toBe(text)
        expect(rendered.props.className).toBe(typographyClasses[variant as keyof typeof typographyClasses])
        expect(vietnameseTextValidation.hasVietnameseCharacters(text)).toBe(true)
      ...
      const longVietnameseText = 'Đây là một đoạn văn bản tiếng Việt rất dài để kiểm tra việc xử lý tràn văn bản khi nội dung vượt quá kích thước cho phép của container'
      const overflowResult = textRenderer.checkTextOverflow(longVietnameseText, 300)
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(overflowResult.truncatedText)).toBe(true)
    })
    ...
    it('should handle Vietnamese text in responsive design', () => {
      ...
      const vietnameseContent = {
        ...
        description: 'Chúng tôi cung cấp dịch vụ sửa chữa laptop toàn diện với chất lượng cao, giá cả hợp lý và thời gian nhanh chóng.',
        features: [
      ...
      const responsiveRenderer = {
        renderForBreakpoint: (content: string, breakpoint: keyof typeof responsiveBreakpoints) => {
          return {
            type: 'div',
            props: {
        ...
        calculateOptimalLineHeight: (textLength: number, breakpoint: keyof typeof responsiveBreakpoints) => {
      ...
      Object.keys(responsiveBreakpoints).forEach(breakpoint => {
        ...
        expect(rendered.props.className).toContain(responsiveBreakpoints[breakpoint as keyof typeof responsiveBreakpoints])
        expect(rendered.props.className).toContain(responsiveTextClasses[breakpoint as keyof typeof responsiveTextClasses])
        expect(rendered.optimizedForVietnamese).toBe(true)
        expect(vietnameseTextValidation.hasVietnameseCharacters(rendered.children)).toBe(true)
      })
      ...
      vietnameseContent.features.forEach(feature => {
        expect(vietnameseTextValidation.hasVietnameseCharacters(feature)).toBe(true)
      })
    })
  })
    ...
    it('should provide proper ARIA labels in Vietnamese', () => {
      ...
      const accessibilityValidator = {
        validateAriaLabels: (component: any) => {
          ...
          return {
            ...
            hasVietnameseContent: component['aria-label']
              ? vietnameseTextValidation.hasVietnameseCharacters(component['aria-label'])
              : false
          }
        },
        ...
        generateScreenReaderText: (action: string, context?: string) => {
          ...
          const template = templates[action as keyof typeof templates] || '{action}'
          return template
    ...
    it('should handle keyboard navigation with Vietnamese tooltips', () => {
      ...
      const keyboardNavigationManager = {
        formatShortcutText: (shortcut: KeyboardShortcut) => {
          const modifierText = shortcut.modifiers.length > 0
        ...
        groupShortcutsByCategory: function() {
      ...
      vietnameseKeyboardShortcuts.forEach(shortcut => {
        expect(vietnameseTextValidation.hasVietnameseCharacters(shortcut.description)).toBe(true)
        expect(vietnameseTextValidation.hasVietnameseCharacters(shortcut.category)).toBe(true)
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(helpText)).toBe(true)
  ...
  describe('1.3.3-UNIT-004: Vietnamese theme and styling validation', () => {
    it('should apply consistent Vietnamese brand styling', () => {
      ...
      const styleGenerator = {
        generateVietnameseButtonStyles: (variant: 'primary' | 'secondary' | 'success' | 'danger') => {
        ...
        getVietnameseStatusColor: (status: string) => {
          ...
          return statusColors[status as keyof typeof statusColors] || brandTheme.colors.primary[500]
        }
      }
      ...
      const primaryButton = styleGenerator.generateVietnameseButtonStyles('primary')
      ...
      const dangerButton = styleGenerator.generateVietnameseButtonStyles('danger')
      ...
      // Test status colors
      expect(styleGenerator.getVietnameseStatusColor('RECEIVED')).toBe(brandTheme.colors.semantic.info)
      expect(styleGenerator.getVietnameseStatusColor('COMPLETED')).toBe(brandTheme.colors.semantic.success)
      expect(styleGenerator.getVietnameseStatusColor('CANCELLED')).toBe(brandTheme.colors.semantic.error)
    ...
    it('should handle dark mode with Vietnamese content', () => {
      ...
      const themeManager = {
        ...
        getThemedStyles: function(component: string, variant?: string) {
          const path = variant
            ? this.darkModeTheme.components[component as keyof typeof this.darkModeTheme.components][variant as any]
            : this.darkModeTheme.components[component as keyof typeof this.darkModeTheme.components]
        ...
        getVietnameseThemeLabels: function() {
          return {
            light: 'Chế độ sáng',
            dark: 'Chế độ tối',
      ...
      Object.values(labels).forEach(label => {
        expect(vietnameseTextValidation.hasVietnameseCharacters(label)).toBe(true)
      })
