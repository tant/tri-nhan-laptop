...
describe('Epic 1.3.2: TanStack Router Integration - Unit Tests', () => {
  ...
  describe('1.3.2-UNIT-001: Route configuration validation', () => {
    it('should validate Vietnamese route paths and metadata', () => {
        ...
        routes: [
          {
            ...
            meta: {
              description: 'Tra cứu tình trạng sửa chữa laptop tại Trí Nhân Laptop',
      ...
      routeConfig.routes.forEach(route => {
        ...
        expect(vietnameseTextValidation.hasVietnameseCharacters(route.title)).toBe(true)
        expect(route.meta.description).toBeDefined()
        expect(vietnameseTextValidation.hasVietnameseCharacters(route.meta.description)).toBe(true)
      })
      ...
      const dashboardRoute = routeConfig.routes.find(r => r.path === '/dashboard')!
    ...
    it('should validate file-based routing structure', () => {
      const fileRoutes = [
        ...
        { file: 'routes/dashboard/index.tsx', path: '/dashboard', component: 'DashboardPage' },
        { file: 'routes/dashboard/customers.tsx', path: '/dashboard/customers', component: 'CustomersPage' },
        { file: 'routes/dashboard/repair-tickets.tsx', path: '/dashboard/repair-tickets', component: 'RepairTicketsPage' },
        { file: 'routes/dashboard/parts.tsx', path: '/dashboard/parts', component: 'PartsPage' },
        { file: 'routes/_authenticated.tsx', path: '/_authenticated', component: 'AuthenticatedLayout' }
      ]
      ...
      const dashboardRoutes = fileRoutes.filter(r => r.path.startsWith('/dashboard'))
      ...
      const authenticatedRoutes = fileRoutes.filter(r => r.path.startsWith('/dashboard'))
  ...
  describe('1.3.2-UNIT-002: Vietnamese URL handling', () => {
    it('should handle Vietnamese search parameters correctly', () => {
      ...
      Object.values(parsedParams).forEach(value => {
        expect(vietnameseTextValidation.hasVietnameseCharacters(value)).toBe(true)
      })
    })
    ...
    it('should handle Vietnamese route parameters', () => {
      ...
      const parsedParams = routeParamHandler.parseParams(generatedPath, customerPattern)

      expect(parsedParams.customerName).toBe(vietnameseCustomerName)
      expect(vietnameseTextValidation.hasVietnameseCharacters(parsedParams.customerName)).toBe(true)
      ...
      const ticketPath = routeParamHandler.buildPath(ticketPattern, { ticketCode })
      const ticketParams = routeParamHandler.parseParams(ticketPath, ticketPattern)
  ...
  describe('1.3.2-UNIT-003: Navigation state management', () => {
    it('should manage navigation history with Vietnamese page titles', () => {
      ...
      const navigationManager = {
        ...
        navigate: function(path: string, title: string, params?: Record<string, string>) {
      ...
      const lastHistoryItem = navigationManager.state.history[navigationManager.state.history.length - 1]
      expect(vietnameseTextValidation.hasVietnameseCharacters(lastHistoryItem.title)).toBe(true)
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(breadcrumbs[1].title)).toBe(true)
    })
    ...
    it('should handle loading states during navigation', () => {
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(loadingMessage)).toBe(true)
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(loadingManager.state.error?.message || '')).toBe(true)
    })
  })
  ...
  describe('1.3.2-UNIT-004: Route guards and authentication', () => {
    it('should validate route access based on user roles', () => {
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(noUserAccess.reason)).toBe(true)
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(staffAccess.reason)).toBe(true)
      ...
      const staffPermissionAccess = routeGuard.checkAccess(staff, permissionRoute)
      expect(staffPermissionAccess.allowed).toBe(false)

      // Test Vietnamese titles based on user role
      const dashboardTitle = routeGuard.getRouteTitle(protectedRoute, shopOwner)
      expect(dashboardTitle).toContain('Chủ cửa hàng')
      expect(vietnameseTextValidation.hasVietnameseCharacters(dashboardTitle)).toBe(true)

      const staffDashboardTitle = routeGuard.getRouteTitle(protectedRoute, staff)
    ...
    it('should handle route transitions with authentication checks', () => {
        ...
        validateTransition: (context: TransitionContext) => {
          ...
          if (from.includes('edit') && to !== from) {
            return {
              ...
              action: 'confirm',
              message: 'Bạn có chắc muốn rời khỏi trang này? Các thay đổi sẽ bị mất.'
            }
          }
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(validResult.reason)).toBe(true)
      ...
      const expiredResult = transitionManager.validateTransition(expiredTransition)
      expect(expiredResult.allowed).toBe(false)
      expect(expiredResult.reason).toBe('Phiên đăng nhập đã hết hạn')
      expect(vietnameseTextValidation.hasVietnameseCharacters(expiredResult.message!)).toBe(true)
      ...
      const unauthorizedResult = transitionManager.validateTransition(unauthorizedTransition)
      expect(unauthorizedResult.allowed).toBe(false)
      expect(unauthorizedResult.action).toBe('redirect')
      expect(vietnameseTextValidation.hasVietnameseCharacters(unauthorizedResult.message!)).toBe(true)

      // Test Vietnamese transition messages
      const transitionMessage = transitionManager.getTransitionMessage('/dashboard', '/customers')
      expect(transitionMessage).toBe('Chuyển đến quản lý khách hàng')
      expect(vietnameseTextValidation.hasVietnameseCharacters(transitionMessage)).toBe(true)
    })
  })
})