...
import { vietnameseTestData, phoneValidation, vietnameseTextValidation } from '../../utils/vietnamese-test-helpers'
...
describe('Epic 1.2.1: Vietnamese Auth Integration - Unit Tests', () => {
  ...
  describe('1.2.1-UNIT-001: Error message translation validation', () => {
    it('should provide Vietnamese translations for all auth error types', () => {
      ...
      Object.entries(authErrorTranslations).forEach(([englishError, vietnameseTranslation]) => {
        expect(vietnameseTranslation).toBeTruthy()
        expect(vietnameseTranslation.length).toBeGreaterThan(0)
        expect(vietnameseTextValidation.hasVietnameseCharacters(vietnameseTranslation)).toBe(true)
      })

      // Test error translation function
      const translateAuthError = (errorCode: string) => {
        return authErrorTranslations[errorCode as keyof typeof authErrorTranslations] || 'Lỗi không xác định'
      }

      expect(translateAuthError('Invalid login credentials')).toBe('Thông tin đăng nhập không chính xác')
    ...
    it('should handle error message context and formatting', () => {
      const formatAuthError = (errorType: string, context?: any) => {
        ...
        let message = baseMessages[errorType as keyof typeof baseMessages] || errorType
      ...
      expect(formatAuthError('password_weak', { min: 8 })).toBe('Mật khẩu cần có ít nhất 8 ký tự')
      expect(formatAuthError('rate_limit', { attempts: 5, wait: 15 })).toBe('Bạn đã thử 5 lần. Vui lòng đợi 15 phút')
      expect(formatAuthError('field_required', { field: 'Email' })).toBe('Trường Email là bắt buộc')
    })
  })
  ...
  describe('1.2.1-UNIT-003: Form label translation validation', () => {
    it('should provide Vietnamese labels for all auth form fields', () => {
      ...
      Object.entries(authFormLabels).forEach(([field, label]) => {
        ...
        if (field !== 'email') { // Email doesn't need Vietnamese characters
    ...
    it('should provide Vietnamese placeholders for form inputs', () => {
      ...
      Object.entries(authPlaceholders).forEach(([field, placeholder]) => {
        ...
        expect(vietnameseTextValidation.hasVietnameseCharacters(placeholder)).toBe(true)
      })
    ...
    it('should categorize and handle different error types', () => {
      const categorizeAuthError = (error: any) => {
        const errorPatterns = {
          ...
          auth: ['credentials', 'unauthorized', 'forbidden', 'expired', 'invalid'],
      ...
      const getVietnameseErrorMessage = (category: string, originalError: any) => {
        ...
        return categoryMessages[category as keyof typeof categoryMessages] || categoryMessages.unknown
      }

      // Test error categorization
      expect(categorizeAuthError({ message: 'Network timeout' })).toBe('network')
      expect(categorizeAuthError({ message: 'Invalid credentials' })).toBe('auth')
      expect(categorizeAuthError({ message: 'Password too weak' })).toBe('validation')
      expect(categorizeAuthError({ message: 'Server internal error' })).toBe('server')
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(networkError)).toBe(true)
    })
    ...
    it('should handle retry logic with Vietnamese feedback', () => {
      const retryManager = {
        ...
        shouldRetry: function(error: any) {
          ...
          return retryableErrors.includes(errorType) && this.currentAttempt < this.maxRetries
        },
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(retryMessage)).toBe(true)

      const maxRetryMessage = retryManager.getMaxRetryMessage()
      expect(maxRetryMessage).toContain('Đã thử')
      expect(vietnameseTextValidation.hasVietnameseCharacters(maxRetryMessage)).toBe(true)
    })
  })
  ...
  describe('1.2.1-UNIT-005: Session storage logic validation', () => {
    it('should handle secure session storage operations', () => {
      const secureStorage = {
        ...
        retrieve(key: string) {
          const mockStorage = new Map()
          mockStorage.set('auth_session', this.encrypt(JSON.stringify({
      ...
      const testData = { user: { email: 'test@trinhanhlaptop.vn' }, token: 'abc123' }
    ...
    it('should validate session expiration and cleanup', () => {
      ...
      expect(vietnameseTextValidation.hasVietnameseCharacters(cleanup.message)).toBe(true)
    })
  })
})