...
} from "@/components/notifications/RealtimeNotifications";
...
describe("RealtimeNotifications Component", () => {
	...
	describe("Basic Rendering", () => {
		...
		it("should apply custom className", () => {
			const { container } = render(
				<RealtimeNotifications className="custom-class" />,
			);
	...
	describe("Event Display", () => {
		it("should display recent events by default", () => {
			...
			expect(
				screen.getByText("Đã tạo phiếu sửa chữa mới: LRP-2025-000001"),
			).toBeInTheDocument();
			expect(
				screen.getByText(
					"Trạng thái đã thay đổi: Tiếp nhận thiết bị → Đang kiểm tra",
				),
			).toBeInTheDocument();
		});
