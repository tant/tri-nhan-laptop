...
describe('useRealtimeUpdates Hook - Basic Tests', () => {
  ...
  describe('Connection Management', () => {
    it('should provide connection status', () => {
      ...
      expect(result.current.connectionStatus).toHaveProperty('connection_count');
  ...
  describe('Statistics and State', () => {
    ...
    it('should track active subscriptions', () => {
      ...
      expect(result.current.activeSubscriptions).toBe(0);
      expect(result.current.subscriptions.filter(sub => sub.active).length).toBe(result.current.activeSubscriptions);
    });

    it('should provide connection status flags', () => {
      const { result } = renderHook(() => useRealtimeUpdates());

      expect(result.current.isConnected).toBe(result.current.connectionStatus.connected);
      expect(result.current.isReconnecting).toBe(result.current.connectionStatus.reconnecting);
    });
  });
