	...
	describe("Ticket Creation Workflow", () => {
		...
		it("should create complete repair ticket with Vietnamese device data", async () => {
			...
			const ticketData = {
				...
				customerDescription:
					"Laptop bị rơi từ bàn xuống, màn hình xuất hiện vết nứt",
				...
				physicalCondition: {
					...
					notes:
						"Chỉ có màn hình bị hỏng, các bộ phận khác hoạt động bình thường",
				},
			};
			...
			await act(async () => {
				for (let i = 0; i < 10; i++) {
					...
					expect(ticket.code).toMatch(
						new RegExp(`^LRP-${currentYear}-\\d{6}$`),
					);
	...
	describe("Vietnamese Problem Documentation", () => {
		it("should handle detailed Vietnamese problem descriptions", async () => {
			...
			const vietnameseProblems = [
				{
					description:
						"Laptop không khởi động được, khi bấm nút nguồn không có phản ứng gì",
					category: "power",
					severity: "critical",
					symptoms: [
						"Không khởi động",
				...
				{
					description:
						"Bàn phím một số phím không hoạt động, đặc biệt là phím cách và enter",
					category: "keyboard",
					severity: "medium",
					symptoms: [
						"Phím cách lỗi",
						"Phím Enter không hoạt động",
						"Gõ bị thiếu ký tự",
					],
				},
				{
			...
			for (const problem of vietnameseProblems) {
				await act(async () => {
					...
					expect(ticket.problem.description).toMatch(
		...
		it("should categorize problems correctly with Vietnamese keywords", async () => {
			...
			for (const test of problemCategories) {
				await act(async () => {
					const category = await result.current.categorizeProblem(
						test.keywords,
					);
	...
	describe("Device Condition Recording", () => {
		it("should record comprehensive device condition assessment", async () => {
			...
			const detailedCondition = {
				...
				notes:
					"Màn hình vỡ ở góc trên phải, cổng USB bên trái bị lỏng, pin chai chỉ dùng được 1 tiếng",
			};
	...
	describe("Draft and Template System", () => {
		...
		it("should create and use repair templates", async () => {
			...
			await act(async () => {
				...
				const ticket = await result.current.createTicketFromTemplate(
					templateId,
					{
						customerPhone: "0901234567",
				...
				await act(async () => {
					try {
						await result.current.createTicket({
							...
							deviceType: "laptop",
							issueDescription:
								field === "issueDescription" ? value : "Valid description",
