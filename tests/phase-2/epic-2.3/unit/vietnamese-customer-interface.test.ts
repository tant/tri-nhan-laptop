  ...
  let mockTickets = [
    {
      ...
      status: 'in_repair',
      device_info: { brand: 'ASUS', model: 'VivoBook', initial_condition: 'Màn hình bị vỡ' },
  ...
  describe('AC4: Vietnamese Customer Interface - Complete Vietnamese localization', () => {
    describe('2.3-UNIT-008: Vietnamese error message generation (P1)', () => {
      ...
      it('should maintain consistent Vietnamese terminology', () => {
        ...
        statuses.forEach(status => {
          const vietnameseLabel = result.current.getVietnameseStatus(status as any)
    ...
    describe('2.3-UNIT-009: Vietnamese status translation (P1)', () => {
      it('should translate all repair statuses to Vietnamese correctly', () => {
        ...
        statusTranslations.forEach(({ en, vn }) => {
          const translation = result.current.getVietnameseStatus(en as any)
      ...
      it('should provide customer-friendly status descriptions', () => {
        ...
        customerFacingStatuses.forEach(({ status, shouldContain }) => {
  ...
  describe('AC10: Error Handling - Vietnamese error messages for validation', () => {
    describe('2.3-UNIT-013: Input validation error generation (P0)', () => {
      it('should generate specific Vietnamese validation errors', () => {
        ...
        phoneValidationCases.forEach(({ input, expected, context }) => {
        ...
        const ticketValidationCases = [
          { input: 'LRP-25-000001', expected: false, context: 'Wrong year format' },
          { input: 'LRP-2025-1234', expected: false, context: 'Too few digits' },
          { input: 'ABC-2025-000001', expected: false, context: 'Wrong prefix' },
          { input: '', expected: false, context: 'Empty' }
        ]

        ticketValidationCases.forEach(({ input, expected, context }) => {
    ...
    describe('2.3-UNIT-015: Error recovery suggestions (P2)', () => {
      ...
      it('should handle edge cases gracefully', () => {
        ...
        expect(result.current.isValidPhoneNumber(null as any)).toBe(false)
        expect(result.current.isValidTicketNumber(undefined as any)).toBe(false)

        // Test whitespace handling
        expect(result.current.isValidPhoneNumber('  0901234567  '.trim())).toBe(true)
        expect(result.current.isValidTicketNumber('  LRP-2025-000001  '.trim())).toBe(true)
      })
    })
  })
})