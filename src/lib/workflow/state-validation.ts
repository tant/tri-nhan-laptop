...
export class StateValidator {
	...
	validateTransition(context: TransitionContext): ValidationResult {
		...
		if (!transitionInfo) {
			...
			result.errors.push(
				`Không thể chuyển từ "${REPAIR_STATES[context.currentState].label}" sang "${REPAIR_STATES[context.targetState].label}"`,
			);
			return result;
		}
	...
	private validateUserPermissions(
		...
		if (!currentStateDefinition.allowedRoles.includes(context.userRole)) {
			result.isValid = false;
			result.errors.push(
				`Bạn không có quyền thay đổi trạng thái "${currentStateDefinition.label}"`,
			);
		}
		...
		if (!targetStateDefinition.allowedRoles.includes(context.userRole)) {
			result.isValid = false;
			result.errors.push(
				`Bạn không có quyền chuyển sang trạng thái "${targetStateDefinition.label}"`,
			);
		}

		// Special role validations
		if (
			context.targetState === "completed" &&
			!["manager", "admin"].includes(context.userRole)
		) {
			result.warnings.push(
				"Trạng thái hoàn thành thường yêu cầu xác nhận từ quản lý",
			);
		}

		if (
			context.targetState === "cancelled_by_customer" &&
			!["receptionist", "manager", "admin"].includes(context.userRole)
		) {
	...
	private validateBusinessRules(
		...
		switch (`${context.currentState}_to_${context.targetState}`) {
			case "awaiting_repair_plan_to_approved_for_repair":
				if (!context.hasCustomerApproval) {
					result.isValid = false;
					result.errors.push(
						"Cần có xác nhận từ khách hàng trước khi duyệt sửa chữa",
					);
			...
			case "approved_for_repair_to_waiting_parts":
				if (context.partsAvailable === true) {
					result.warnings.push(
						"Linh kiện đã có sẵn, có thể chuyển thẳng sang sửa chữa",
					);
				}
				break;
			...
			case "quality_testing_to_ready_for_pickup":
				if (!context.qualityCheckPassed) {
					...
					result.requiredActions.push(
						"Hoàn thành tất cả các bước kiểm tra chất lượng",
					);
				}
				break;
			...
			case "ready_for_pickup_to_abandoned": {
				...
				if (contactAttempts < 3) {
					result.isValid = false;
					result.errors.push(
						"Cần ít nhất 3 lần liên hệ khách hàng trước khi coi là bỏ lại",
					);
			...
			case "delivered_to_warranty_claim": {
				...
				if (daysSinceDelivery > 90) {
					result.warnings.push(
						"Đã quá thời gian bảo hành thông thường (90 ngày)",
					);
				}
				break;
			}
		...
		if (
			REPAIR_STATES[context.currentState].isTerminal &&
			context.targetState !== "warranty_claim"
		) {
			result.warnings.push(
				"Đang chuyển từ trạng thái đã kết thúc. Cần xác nhận đặc biệt.",
			);
		}
	}
	...
	private validateStateRequirements(
		...
		if (
			targetStateDefinition.requiresCustomerApproval &&
			!context.hasCustomerApproval
		) {
			result.isValid = false;
			result.errors.push(
				`Trạng thái "${targetStateDefinition.label}" yêu cầu xác nhận từ khách hàng`,
			);
		...
		if (targetStateDefinition.requiresPayment && !context.hasPayment) {
			result.isValid = false;
			result.errors.push(
				`Trạng thái "${targetStateDefinition.label}" yêu cầu thanh toán`,
			);
	...
	private validateCondition(
		...
		switch (condition) {
			case "parts_needed":
				if (context.partsAvailable === true) {
					result.warnings.push(
						"Linh kiện đã có sẵn, có thể bỏ qua việc chờ linh kiện",
					);
				}
				break;
			...
			case "diagnosis_needed":
				if (context.metadata?.diagnosisComplete) {
					result.warnings.push(
						"Chẩn đoán đã hoàn thành, có thể chuyển thẳng sang sửa chữa",
					);
				}
				break;
			...
			case "payment_incomplete":
				if (context.hasPayment) {
					result.warnings.push(
						"Thanh toán đã hoàn thành, có thể chuyển thẳng sang hoàn thành",
					);
				}
				break;
		}
	...
	private addWarningsAndRecommendations(
		...
		if (
			daysInCurrentState > 7 &&
			currentStateDefinition.category === "active"
		) {
			result.warnings.push(
				`Đã ở trạng thái "${currentStateDefinition.label}" quá 7 ngày`,
			);
		}
		...
		if (targetStateDefinition.autoAdvanceAfter) {
			result.warnings.push(
				`Trạng thái này sẽ tự động chuyển sau ${targetStateDefinition.autoAdvanceAfter} phút`,
			);
		}

		// Quality recommendations
		if (
			context.targetState === "in_repair" &&
			!context.metadata?.repairPlanApproved
		) {
			result.warnings.push(
				"Nên có kế hoạch sửa chữa chi tiết trước khi bắt đầu",
			);
		}

		if (
			context.targetState === "delivered" &&
			!context.metadata?.customerSatisfactionChecked
		) {
		...
		if (context.targetState === "on_hold") {
			result.requiredActions.push(
				"Ghi rõ lý do tạm dừng và thời gian dự kiến tiếp tục",
			);
		}

		if (context.targetState === "warranty_claim") {
			result.requiredActions.push(
				"Xác minh thời hạn bảo hành và điều kiện áp dụng",
			);
		}
	}
	...
	validateBulkTransition(
		...
		success: TransitionContext[];
		failed: Array<{ context: TransitionContext; result: ValidationResult }>;
	} {
	...
	getRecommendedNextStates(
		context: Omit<TransitionContext, "targetState">,
	): RepairState[] {
		const validTransitions = STATE_TRANSITIONS.filter(
			(t) => t.from === context.currentState,
		).map((t) => t.to);
