...
export const REPAIR_STATES: Record<RepairState, RepairStateDefinition> = {
  ...
  in_diagnosis: {
    ...
    description: 'Đang thực hiện chẩn đoán chi tiết và xác định nguyên nhân lỗi',
    color: '#6366F1', // indigo
    category: 'active',
    isTerminal: false,
    requiresCustomerApproval: false,
  ...
  quality_testing: {
    ...
    requiresPayment: false,
    notifyCustomer: false,
    allowedRoles: ['quality_controller', 'senior_technician', 'manager', 'admin']
  },
  ready_for_pickup: {
    id: 'ready_for_pickup',
    label: 'Sẵn sàng giao',
    description: 'Thiết bị đã hoàn thành sửa chữa và sẵn sàng để khách hàng nhận',
  ...
  abandoned: {
    id: 'abandoned',
    label: 'Bỏ lại',
    description: 'Khách hàng không liên lạc hoặc không đến nhận thiết bị sau thời gian quy định',
...
export const STATE_TRANSITIONS: StateTransition[] = [
  // From device_received
  {
    ...
    businessRules: ['Device must be logged in system', 'Initial condition documented'],
  ...
  },
  {
    ...
    to: 'cancelled_by_customer',
    requiresConfirmation: true,
    businessRules: ['Customer no longer wants to wait', 'Parts taking too long']
  },
  {
    from: 'waiting_parts',
    to: 'on_hold',
  ...
  },
  {
    ...
    to: 'abandoned',
    requiresConfirmation: true,
    businessRules: ['Customer no contact for 30 days', 'Multiple pickup attempts failed']
  },
...
export function getTransitionInfo(from: RepairState, to: RepairState): StateTransition | undefined {
  return STATE_TRANSITIONS.find(
    transition => transition.from === from && transition.to === to
  );
}

export function getStatesByCategory(category: RepairStateDefinition['category']): RepairState[] {
...
export type RepairType = 'hardware' | 'software' | 'screen' | 'liquid_damage' | 'battery' | 'motherboard';
