...
export function translateAuthError(error: AuthError | Error | string): string {
  ...
  if (lowerMessage.includes('account is disabled') || lowerMessage.includes('user is disabled')) {
  ...
  if (lowerMessage.includes('not authenticated') || lowerMessage.includes('authentication required')) {
    return 'Bạn cần đăng nhập để sử dụng chức năng này.';
  }

  if (lowerMessage.includes('insufficient permissions') || lowerMessage.includes('unauthorized')) {
  ...
  if (lowerMessage.includes('password reset') && lowerMessage.includes('expired')) {
...
export const authSuccessMessages = {
  ...
  emailConfirmed: 'Email đã được xác nhận thành công! Bây giờ bạn có thể đăng nhập.',
  profileUpdated: 'Thông tin cá nhân đã được cập nhật thành công.',
  accountCreated: 'Tài khoản đã được tạo thành công! Vui lòng kiểm tra email để xác nhận tài khoản.',
} as const;
 ...
 */
export function getValidationMessage(field: keyof typeof authValidationMessages): string {
 ...
 */
export function getSuccessMessage(type: keyof typeof authSuccessMessages): string {
