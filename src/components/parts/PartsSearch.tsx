			...
			const timeout = setTimeout(() => {
				const filteredParts = parts.filter((part) => {
					...
					const matchesCategory =
						...
						part.category === searchFilters.category;

					// Brand filter
					const matchesBrand =
						searchFilters.brand === "all" || part.brand === searchFilters.brand;
					...
					const matchesStock =
						...
						(searchFilters.stockStatus === "in_stock" &&
							part.current_stock > minStock) ||
						(searchFilters.stockStatus === "low_stock" &&
							part.current_stock <= minStock &&
							part.current_stock > 0) ||
						(searchFilters.stockStatus === "out_of_stock" &&
							part.current_stock === 0);

					// Price range filter
					const matchesPrice =
						part.unit_price >= searchFilters.priceRange[0] &&
						part.unit_price <= searchFilters.priceRange[1];

					// Supplier filter
					const matchesSupplier =
						searchFilters.supplier === "all" ||
						part.supplier_info
							?.toLowerCase()
					...
					const matchesModel =
						searchFilters.modelCompatibility === "" ||
						part.model_compatibility?.some((model) =>
							model
								.toLowerCase()
								.includes(searchFilters.modelCompatibility.toLowerCase()),
						);
					...
					<div className="flex flex-wrap gap-2">
						{POPULAR_LAPTOP_MODELS.map((model) => (
							<Button
								key={model}
								variant={
									filters.modelCompatibility === model ? "default" : "outline"
								}
								size="sm"
								onClick={() =>
									updateFilter(
				...
				{showAdvancedFilters && (
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 border-t">
						...
						<div className="md:col-span-2">
							<Label className="text-sm font-medium mb-2 block">
								Khoảng giá: {formatPrice(filters.priceRange[0])} -{" "}
						...
						<div>
							...
							<Input
								...
								onChange={(e) =>
									updateFilter("modelCompatibility", e.target.value)
								}
							/>
						</div>
					</div>
