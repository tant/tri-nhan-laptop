...
export function BulkImportModal({
		...
		for (let i = 0; i < updatedData.length; i++) {
			...
			if (validationError) {
				...
				try {
					...
					const { error } = await supabase.from("parts").insert([row.data]);
	...
	const downloadTemplate = () => {
		...
		const sampleData = [
			...
			"Memory",
			"Samsung",
			"Bộ nhớ DDR4 8GB 3200MHz",
			"1500000",
			"1200000",
			"1800000",
			"10",
			"5",
			"Công ty ABC - 0123456789",
			"Kệ A-1-3",
			"Dell Inspiron 15;HP Pavilion 14",
			"24",
			"new",
		];
												...
												<div key={row.row} className="p-3 border rounded-lg">
													<div className="grid grid-cols-2 gap-2 text-sm">
														...
														<div>
															...
															{formatPrice(row.data.unit_price || 0)}
														</div>
													</div>
												</div>
											))}
								...
								<CardContent className="space-y-4">
									<div className="space-y-2">
										<div className="flex justify-between text-sm">
											<span>
												Tiến độ: {stats.processed}/{stats.total}
											</span>
											<span>
