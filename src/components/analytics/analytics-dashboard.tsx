  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Tổng cộng {total} phiếu sửa chữa
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          {data.filter(item => item.count > 0).map((item) => (
            <div key={item.status} className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium">{item.vietnameseName}</span>
              </div>
            ...
            <div key={customer.id} className="flex items-center justify-between p-3 border rounded-lg">
              <div className="flex items-center gap-3">
                <div className="flex items-center justify-center w-8 h-8 bg-[#299fce]/10 rounded-full">
                  <span className="text-sm font-bold text-[#299fce]">#{index + 1}</span>
                </div>
                <div>
              ...
              <div className="text-right">
                <p className="font-bold text-[#299fce]">{formatCurrency(customer.totalSpent)}</p>
      ...
      <CardContent>
        <div className="space-y-3">
          {data.map((device, index) => (
            <div key={`${device.deviceType}-${device.deviceModel}`} className="flex items-center justify-between p-3 border rounded-lg">
              <div className="flex items-center gap-3">
              ...
              <div className="text-right">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-medium">{formatCurrency(device.averageCost)}</p>
                    <p className="text-xs text-muted-foreground">Chi phí TB</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium">{device.averageTime.toFixed(1)} ngày</p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Dữ liệu từ {dateRange.from.toLocaleDateString("vi-VN")} đến {dateRange.to.toLocaleDateString("vi-VN")}
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          icon={Package}
          className={metrics.lowStockItems > 0 ? "border-orange-200 bg-orange-50" : ""}
        />
      </div>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between p-3 border rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Toàn bộ linh kiện</p>
                  </div>
                ...
                {metrics.lowStockItems > 0 && (
                  <div className="flex items-center justify-between p-3 border border-orange-200 bg-orange-50 rounded-lg">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="font-medium text-orange-800">Cảnh báo tồn kho</p>
                        <p className="text-sm text-orange-600">Cần đặt hàng bổ sung</p>
                      </div>
                    </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center p-4 border rounded-lg">
                    <p className="text-2xl font-bold text-[#299fce]">{metrics.newCustomers}</p>
                    <p className="text-sm text-muted-foreground">Khách hàng mới</p>
                  </div>
                  <div className="text-center p-4 border rounded-lg">
                    <p className="text-2xl font-bold text-green-600">{metrics.returningCustomers}</p>
                    <p className="text-sm text-muted-foreground">Khách hàng quay lại</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-medium">
                      {formatPercentage(metrics.totalCustomers > 0 ? (metrics.newCustomers / metrics.totalCustomers) * 100 : 0)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm">Tỷ lệ khách hàng quay lại</span>
                    <span className="text-sm font-medium">
                      {formatPercentage(metrics.totalCustomers > 0 ? (metrics.returningCustomers / metrics.totalCustomers) * 100 : 0)}
                    </span>
                  </div>
                  <div className="flex justify-between">
        ...
        <TabsContent value="devices" className="space-y-4">
          <PopularDevices data={popularDevices} formatCurrency={formatCurrency} />
        </TabsContent>
