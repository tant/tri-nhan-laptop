...
import { ConflictInfo } from '@/hooks/use-sync-manager';
...
export function ConflictResolver({ open, onOpenChange, conflict }: ConflictResolverProps) {
  const { applyConflictResolution } = useSyncManager();
  const [selectedStrategy, setSelectedStrategy] = useState<ConflictInfo['resolution_strategy']>('remote_wins');
  ...
  const calculateDifferences = (): ConflictDiff[] => {
    ...
    for (const key of allKeys) {
      ...
      if (localValue === undefined && remoteValue !== undefined) {
        diffs.push({ field: key, local_value: localValue, remote_value: remoteValue, type: 'added' });
      } else if (localValue !== undefined && remoteValue === undefined) {
        diffs.push({ field: key, local_value: localValue, remote_value: remoteValue, type: 'removed' });
      } else if (JSON.stringify(localValue) !== JSON.stringify(remoteValue)) {
        diffs.push({ field: key, local_value: localValue, remote_value: remoteValue, type: 'modified' });
      } else {
        diffs.push({ field: key, local_value: localValue, remote_value: remoteValue, type: 'same' });
      }
    }
  ...
  React.useEffect(() => {
    if (conflict) {
      setMergedData({ ...(conflict.local_data || {}), ...(conflict.remote_data || {}) });
    }
  }, [conflict]);
  ...
  const handleFieldMerge = (field: string, value: any, source: 'local' | 'remote') => {
    setMergedData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleResolve = async () => {
    ...
    try {
      ...
      await applyConflictResolution(conflict.id, resolvedData, selectedStrategy);
      onOpenChange(false);
    } catch (error) {
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="bg-amber-50 p-4 rounded-lg border border-amber-200">
            <div className="flex items-center gap-2 mb-2">
              <span className="font-medium text-amber-800">Thông tin xung đột:</span>
            </div>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="font-medium">Entity ID:</span> {conflict.entity_id}
              </div>
              <div>
                <span className="font-medium">Loại:</span> {conflict.entity_id.split('-')[0]}
              </div>
              <div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button
                variant={selectedStrategy === 'remote_wins' ? 'default' : 'outline'}
                size="sm"
          ...
          <Tabs defaultValue="differences" className="flex-1">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="differences">So sánh ({differences.filter(d => d.type !== 'same').length})</TabsTrigger>
                  ...
                  {differences.map((diff, index) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline" className="text-xs">
                            {diff.type === 'added' ? 'Thêm mới' :
                             diff.type === 'removed' ? 'Đã xóa' :
                             diff.type === 'modified' ? 'Đã sửa' : 'Giống nhau'}
                          </Badge>
                        </div>

                        {diff.type !== 'same' && selectedStrategy === 'merge' && (
                          <div className="flex gap-1">
                            {diff.local_value !== undefined && (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleFieldMerge(diff.field, diff.local_value, 'local')}
                                className="text-xs"
                              >
                                Chọn cục bộ
                              </Button>
                            ...
                            {diff.remote_value !== undefined && (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleFieldMerge(diff.field, diff.remote_value, 'remote')}
                                className="text-xs"
                              >
                                Chọn từ xa
                              </Button>
                      ...
                      {diff.type !== 'same' && (
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            <div className="font-medium text-blue-700">Cục bộ (v{conflict.local_version}):</div>
                            <div className="bg-blue-50 p-2 rounded mt-1 min-h-8">
                              {formatValue(diff.local_value)}
                            </div>
                          </div>
                          <div>
                            <div className="font-medium text-green-700">Từ xa (v{conflict.remote_version}):</div>
...
export function ConflictList({ className }: { className?: string }) {
  ...
  const handleAutoResolve = async (conflict: ConflictInfo) => {
    try {
      ...
      await applyConflictResolution(conflict.id, conflict.remote_data, 'remote_wins');
    } catch (error) {
