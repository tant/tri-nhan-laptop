...
export function SyncStatusIndicator({
  ...
  const getStatusColor = () => {
    ...
    switch (connectionState.status) {
      case 'connected':
        return connectionState.quality === 'excellent' ? 'text-green-500' :
               connectionState.quality === 'good' ? 'text-green-400' :
               connectionState.quality === 'poor' ? 'text-yellow-500' : 'text-orange-500';
      case 'connecting':
      case 'reconnecting':
      ...
      <div className={cn(
            ...
            <div className="border-t pt-2 mt-2 space-y-1">
              ...
              {syncStats.sync_lag > 0 && (
                <div className="flex justify-between text-xs">
                  ...
                  <span className={cn(
                    'font-medium',
                    syncStats.sync_lag < 100 ? 'text-green-600' :
                    syncStats.sync_lag < 500 ? 'text-yellow-600' : 'text-red-600'
                  )}>
                    ...
                    <div key={session.id} className="flex items-center gap-2 text-xs">
                      ...
                      <span className="text-gray-500 text-xs">
                        {session.context.current_page && (
                          session.context.current_page.split('/').pop() || 'dashboard'
                        )}
                      </span>
                    </div>
                  ))}
        ...
        {syncStats.last_sync && isConnected && (
          <div className="border-t pt-2 mt-1">
            <div className="text-xs text-gray-500 text-center">
              Đồng bộ lần cuối: {new Date(syncStats.last_sync).toLocaleTimeString('vi-VN')}
            </div>
          </div>
        )}
      </div>
...
export function CompactSyncStatusIndicator({ className }: { className?: string }) {
  const { isConnected, isReconnecting, hasUnresolvedConflicts, connectionState } = useSyncManager();

  return (
    <div className={cn('flex items-center gap-1', className)}>
      <div className={cn(
        'w-2 h-2 rounded-full',
        hasUnresolvedConflicts ? 'bg-amber-500' :
        isReconnecting ? 'bg-blue-500 animate-pulse' :
        isConnected ? 'bg-green-500' : 'bg-red-500'
      )}></div>

      <span className={cn(
        'text-xs',
        hasUnresolvedConflicts ? 'text-amber-600' :
        isReconnecting ? 'text-blue-600' :
        isConnected ? 'text-green-600' : 'text-red-600'
      )}>
        {hasUnresolvedConflicts ? 'Xung đột' :
         isReconnecting ? 'Đang kết nối...' :
