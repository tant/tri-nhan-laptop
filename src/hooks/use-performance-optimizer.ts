...
import { SyncEvent } from './use-sync-manager';
...
export function usePerformanceOptimizer() {
  ...
  const eventHandlersRef = useRef<Map<string, (events: SyncEvent[]) => void>>(new Map());
  ...
  const getPerformanceMessage = (type: 'optimization' | 'batch' | 'throttle' | 'cleanup'): string => {
    switch (type) {
      case 'optimization': return 'Đã tối ưu hóa hiệu suất';
      case 'batch': return 'Xử lý theo lô';
      case 'throttle': return 'Giới hạn tần suất cập nhật';
      case 'cleanup': return 'Dọn dẹp bộ nhớ';
      default: return 'Tối ưu hóa không xác định';
    }
  };

  // Calculate event priority
  const getEventPriority = useCallback((event: SyncEvent): EventBatch['priority'] => {
    if (config.priority_events.includes(event.type)) {
      return 'high';
    }

    // Check if event affects currently visible data
    if (event.entity_type === 'repair_ticket' &&
  ...
  // Process event batch
  const processBatch = useCallback(async (events: SyncEvent[]) => {
    ...
    try {
      ...
      setMetrics(prev => ({
        ...
        average_batch_size: processingStatsRef.current.total_events_in_batches /
                           processingStatsRef.current.batches_processed,
        processing_latency: processingStatsRef.current.processing_times
          .reduce((a, b) => a + b, 0) / processingStatsRef.current.processing_times.length,
  ...
  const addEventToBatch = useCallback((event: SyncEvent) => {
  ...
  const scheduleUIUpdate = useCallback((updateFunction: () => void) => {
    ...
    if (!queue.scheduled) {
      ...
      const delay = Math.max(0, config.ui_update_throttle - timeSinceLastUpdate);
  ...
  const throttledEventProcessor = useCallback((event: SyncEvent) => {
  ...
  const processEvent = useCallback((event: SyncEvent, options?: {
  ...
  const getPerformanceRecommendations = useCallback((): string[] => {
    ...
    if (metrics.ui_update_frequency > 30) {
      recommendations.push('Tăng thời gian throttle UI để giảm tần suất cập nhật');
    }
    ...
    if (metrics.events_throttled > metrics.events_processed * 0.5) {
      recommendations.push('Xem xét tăng batch size để xử lý nhiều sự kiện hơn');
    }
