...
export function useRepairTicketsCrud() {
	...
	const { validateTicketData, getConditionDescription, inferRepairCategory } =
		useRepairValidation();
			...
			try {
				...
				const ticketInsert = {
					...
					estimated_completion_date:
						ticketData.estimatedCompletion?.toISOString(),
				...
				if (
					...
					const serviceNote = [
						...
						ticketData.physicalCondition.notes &&
							`Tình trạng vật lý: ${ticketData.physicalCondition.notes}`,
					]
						.filter(Boolean)
						.join("\n");
					...
					if (noteError) {
						warnings.push(
							`Không thể thêm ghi chú dịch vụ: ${noteError.message}`,
						);
					}
				}
	...
	const getRepairById = useCallback(
		async (ticketId: string): Promise<RepairTicketWithDetails | null> => {
			try {
				setLoading(true);
				setError(null);
				...
				if (queryError) {
					...
					throw new Error(
						`Không thể tải thông tin phiếu sửa chữa: ${queryError.message}`,
					);
				}
	...
	const loadDraft = useCallback(
		async (draftId: string): Promise<Partial<NewRepairTicket> | null> => {
			try {
				setLoading(true);
				setError(null);
	...
	const deleteTicket = useCallback(
		async (ticketId: string): Promise<boolean> => {
			try {
				...
				if (updateError) {
					throw new Error(
						`Không thể xóa phiếu sửa chữa: ${updateError.message}`,
					);
				}
