...
export function useCustomerPortal() {
	...
	const lookupRepair = useCallback(
		async (ticketNumber: string, customerPhone: string) => {
			setState((prev) => ({
				...prev,
				loading: true,
				error: null,
	...
	const lookupRepairByEmail = useCallback(
		async (_customerEmail: string, customerPhone: string) => {
			setState((prev) => ({
				...prev,
				loading: true,
				error: null,
	...
	const submitFeedback = useCallback(
		async (
			...
			try {
				...
				if (verifyError) {
					throw new Error(
						"Không tìm thấy phiếu sửa chữa hoặc không có quyền truy cập",
					);
				}
	...
	const subscribeToRepairUpdates = useCallback(
		(repairId: string, callback: (repairInfo: CustomerRepairInfo) => void) => {
