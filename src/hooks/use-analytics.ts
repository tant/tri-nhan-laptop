...
export function useAnalytics(dateRange?: { from: Date; to: Date }) {
	...
	const calculateMetrics = useCallback(async () => {
        ...
        `)
			...
			const totalRevenue =
				repairs?.reduce((sum, repair) => sum + (repair.final_cost || 0), 0) ||
				0;
			const previousRevenue =
				previousRepairs?.reduce(
					(sum, repair) => sum + (repair.final_cost || 0),
					0,
				) || 0;
			const revenueGrowth =
				previousRevenue > 0
			...
			const pendingRepairs =
				repairs?.filter((r) => !["delivered", "cancelled"].includes(r.status))
					.length || 0;

			// Calculate average repair time (simplified)
			const completedWithTime =
				repairs?.filter((r) => r.status === "delivered" && r.completed_at) ||
				[];
			...
			const lowStockItems =
				parts?.filter((p) => p.current_stock <= p.min_stock_level).length || 0;
			const totalInventoryValue =
				parts?.reduce((sum, p) => sum + p.current_stock * p.selling_price, 0) ||
				0;

			// Calculate parts usage value
			const partsUsageValue =
				repairs?.reduce((sum, repair) => {
					return (
						sum +
						(repair.repair_parts?.reduce(
							(partSum: number, part: any) => partSum + (part.total_cost || 0),
							0,
						) || 0)
					);
				}, 0) || 0;
			...
			const technicianEfficiency =
				averageRepairTime > 0 ? Math.max(0, 100 - averageRepairTime * 10) : 80;
	...
	const getRevenueData = useCallback(
		async (period: "daily" | "weekly" | "monthly" = "daily") => {
			try {
				...
				const revenueData: RevenueData[] = Object.entries(groupedData).map(
					([period, data]) => ({
						period,
	...
	const getStatusDistribution = useCallback(async () => {
		try {
			...
			const statusDistribution: RepairStatusDistribution[] = Object.entries(
				statusCounts,
			).map(([status, count]) => ({
	...
	const getPopularDevices = useCallback(async () => {
		try {
			const { data: repairs } = await supabase
				.from("repairs")
				.select(
					"device_type, device_model, final_cost, created_at, completed_at",
				)
				.gte("created_at", range.from.toISOString())
				.lte("created_at", range.to.toISOString());
					...
					return {
						...
						averageTime:
							data.completedCount > 0
								? data.totalTime / data.completedCount
								: 0,
					};
				})
