...
export interface RealtimeEvent {
  id: string;
  type: 'ticket_created' | 'status_changed' | 'ticket_updated' | 'notification_sent';
  timestamp: string;
  data: any;
  message: string;
}
...
export function useRealtimeUpdates() {
  ...
  const getVietnameseMessage = (type: string, data: any): string => {
    switch (type) {
      case 'ticket_created':
        ...
        (payload) => {
          ...
          if (oldTicket.current_state !== updatedTicket.current_state) {
            addEvent('status_changed', {
              ...
              from_state: REPAIR_STATES[oldTicket.current_state]?.label || oldTicket.current_state,
              to_state: REPAIR_STATES[updatedTicket.current_state]?.label || updatedTicket.current_state,
      ...
      .on('postgres_changes',
        ...
        (payload) => {
          ...
          addEvent('status_changed', {
            ...
            from_state: REPAIR_STATES[statusChange.from_state]?.label || statusChange.from_state,
            to_state: REPAIR_STATES[statusChange.to_state]?.label || statusChange.to_state,
  ...
  const onEvent = useCallback((handler: (event: RealtimeEvent) => void): string => {
  ...
  }, [subscribeToTickets, subscribeToStatusChanges, subscribeToNotifications, subscribeToCustomNotifications]);
  ...
  // Get events by type
  const getEventsByType = useCallback((type: RealtimeEvent['type']): RealtimeEvent[] => {
    return events.filter(event => event.type === type);
  }, [events]);

  // Get recent events
  const getRecentEvents = useCallback((minutes: number = 5): RealtimeEvent[] => {
