...
export function useConnectionManager() {
	...
	const getConnectionMessage = (event: ConnectionHistory["event"]): string => {
		switch (event) {
			...
			case "disconnected":
				return "Mất kết nối";
			case "reconnecting":
				return "Đang thử kết nối lại";
			case "failed":
				return "Kết nối thất bại";
			case "degraded":
				return "Chất lượng kết nối giảm";
			default:
	...
	const addConnectionEvent = useCallback(
		(
			...
			window.dispatchEvent(
				new CustomEvent("connection-event", {
					detail: {
						...historyEntry,
	...
	const calculateMetrics = useCallback(async () => {
		...
		const throughput =
			latency < 100 ? 100 : latency < 300 ? 50 : latency < 1000 ? 25 : 10;
	...
	const calculateReconnectionDelay = useCallback(
		(attempt: number): number => {
			const { initial_delay, max_delay, backoff_multiplier, jitter } =
				reconnectionConfig;
