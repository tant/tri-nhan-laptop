...
export function useSyncManager() {
  ...
  const conflictHandlers = useRef<Map<string, (conflict: ConflictInfo) => Promise<any>>>(new Map());

  // Vietnamese status messages
  const getSyncMessage = (type: string, data: any): string => {
    switch (type) {
      case 'status_update':
  ...
  // Initialize session
  const initializeSession = useCallback(async (userId: string, userName: string) => {
    try {
  ...
  const resolveConflictAutomatically = useCallback(async (
    ...
    if (local_data.updated_at && remote_data.updated_at) {
      ...
      if (Math.abs(remoteTime - localTime) > 5000) { // More than 5 seconds apart
  ...
  const applyConflictResolution = useCallback(async (
    conflictId: string,
    resolvedData: any,
    ...
    try {
      ...
      const tableName = getTableFromEntityType(conflict.entity_id.split('-')[0] as any);
      const { error } = await supabase
        .from(tableName)
        .update({
          ...resolvedData,
  ...
  const broadcastSyncEvent = useCallback(async (
    type: SyncEvent['type'],
  ...
  // Utility functions
  const getTableFromEntityType = (entityType: SyncEvent['entity_type']): string => {
    switch (entityType) {
      case 'repair_ticket': return 'repair_tickets';
      case 'customer': return 'customers';
      case 'part_usage': return 'part_usage';
      case 'comment': return 'ticket_comments';
      default: return '';
    }
  };

  const mergeArraysByUniqueId = (localArray: any[], remoteArray: any[]): any[] => {
  ...
  useEffect(() => {
    const statsInterval = setInterval(() => {
      setSyncStats(prev => ({
        ...prev,
        connection_uptime: connectionState.status === 'connected'
          ? Date.now() - new Date(connectionState.last_connected || 0).getTime()
          : 0,
