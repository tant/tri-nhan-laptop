...
export function useCostTracking() {
	...
	const addCostBreakdown = useCallback(
		async (
			repairId: string,
	...
	const removeCostBreakdown = useCallback(
		async (breakdownId: string): Promise<boolean> => {
			try {
				setLoading(true);
	...
	const getRepairCostBreakdown = useCallback(
		async (repairId: string): Promise<CostBreakdown[]> => {
			try {
				const { data, error } = await supabase
					.from("repair_cost_breakdown")
					.select("*")
	...
	const calculateRepairCosts = useCallback(
		async (repairId: string): Promise<CostSummary | null> => {
			try {
				const { data, error } = await supabase.rpc("calculate_repair_costs", {
	...
	const updateRepairTotals = useCallback(
		async (repairId: string): Promise<boolean> => {
			try {
				const { data, error } = await supabase.rpc("update_repair_totals", {
					repair_uuid: repairId,
				});
	...
	const generateCustomerQuote = useCallback(
		async (
			repairId: string,
			userId: string,
			terms?: string,
			...
			try {
				...
				const { data, error } = await supabase.rpc("generate_customer_quote", {
					repair_uuid: repairId,
					user_uuid: userId,
					quote_terms: terms,
	...
	const getCustomerQuotes = useCallback(
		async (repairId: string): Promise<CustomerQuote[]> => {
			try {
				const { data, error } = await supabase
	...
	const getCostChangeHistory = useCallback(
		async (repairId: string): Promise<CostChangeHistory[]> => {
			try {
				const { data, error } = await supabase
	...
	// Approve cost change
	const approveCostChange = useCallback(
		async (changeId: string, approvedBy: string): Promise<boolean> => {
			try {
				setLoading(true);
	...
	// Add labor cost
	const addLaborCost = useCallback(
		async (
			repairId: string,
	...
	// Add overhead cost
	const addOverheadCost = useCallback(
		async (
			repairId: string,
